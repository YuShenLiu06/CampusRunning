...
class PaceFluctuator:
    ...
    def __init__(self, 
        ...
        """
        self.base_pace = base_pace_min_per_km
        self.fluctuation_intensity = fluctuation_intensity
    ...
    def _get_phase_multiplier(self, progress: float) -> float:
        ...
        # 稳定阶段：配速接近目标（±5%）
            ...
            # 在目标配速附近小幅波动
            steady_progress = (progress - self.phases['steady'][0]) / (self.phases['steady'][1] - self.phases['steady'][0])
            return 1.0 + 0.05 * math.sin(steady_progress * math.pi * 4)
        
        # 疲劳阶段：配速逐渐减慢（比目标配速慢5-15%）
        elif progress <= self.phases['fatigue'][1]:
            # 从慢5%逐渐到慢15%
            fatigue_progress = (progress - self.phases['fatigue'][0]) / (self.phases['fatigue'][1] - self.phases['fatigue'][0])
        ...
        # 最后阶段：可能冲刺或保持
            ...
            if random.random() < 0.5:
                # 短暂冲刺：配速接近目标
                final_progress = (progress - self.phases['final'][0]) / (self.phases['final'][1] - self.phases['final'][0])
    ...
    def _adjust_pace_profile(self, pace_profile: List[float]) -> List[float]:
        ...
        # 应用调整因子，但限制调整幅度以保持波动性
        max_adjustment = 1.1  # 最大调整10%
        adjustment_factor = max(1/max_adjustment, min(max_adjustment, adjustment_factor))
    ...
    def generate_segment_times(self, pace_profile: List[float], 
                              segment_distances: List[float]) -> List[float]:
        """
        根据配速曲线和分段距离计算每段的时间
        ...
        if len(pace_profile) != len(segment_distances):
            raise ValueError("配速曲线和分段距离数量不匹配")
